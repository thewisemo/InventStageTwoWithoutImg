...
public class ItemProvider extends ContentProvider {
    ...
    public boolean onCreate() {
        // Create and initialize an ItemDbHelper object to gain access to the items database.
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (match) {
            ...
            case ITEM_ID:
                ...
                // For an example URI such as "content://com.example.wisemo.myinventory/items/3",
    ...
    private Uri insertItem(Uri uri, ContentValues values) {
        ...
        if (availability == null || !ItemContract.ItemEntry.isValidAvailability(availability)) {
            throw new IllegalArgumentException("The product requires valid availability");
        }

        // If the price is provided, check that it's greater than or equal to 80 EGP
        Integer price = values.getAsInteger(ItemContract.ItemEntry.COLUMN_ITEM_PRICE);
        if (price != null && price < 80) {
            throw new IllegalArgumentException("The price can't be less than 80 EGP");
        }

        // If the quantity is provided, check that it's greater than or equal to 50 EGP
        Integer quantity = values.getAsInteger(ItemContract.ItemEntry.COLUMN_ITEM_QUANTITY);
        if (quantity != null && quantity < 0) {
            throw new IllegalArgumentException("The quantity can't be less than 0 item");
        }
    ...
    private int updateItem(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        // If the {@link ItemEntry#COLUMN_ITEM_AVAILABILITY} key is present,
        // check that the availability value is valid.
        if (values.containsKey(ItemContract.ItemEntry.COLUMN_ITEM_AVAILABILITY)) {
            Integer availability = values.getAsInteger(ItemContract.ItemEntry.COLUMN_ITEM_AVAILABILITY);
            if (availability == null || !ItemContract.ItemEntry.isValidAvailability(availability)) {
                throw new IllegalArgumentException("The product requires valid availability");
            }
        ...
        if (values.containsKey(ItemContract.ItemEntry.COLUMN_ITEM_PRICE)) {
            // Check that the price is greater than or equal to 80 EGP
            Integer price = values.getAsInteger(ItemContract.ItemEntry.COLUMN_ITEM_PRICE);
            if (price != null && price < 80) {
                throw new IllegalArgumentException("The price can't be less than 80 EGP");
            }
